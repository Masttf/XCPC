...
void solve(){
	...
	auto getVal = [&](int x) -> int{
		...
		else if(a[x - 1] <= a[x] && a[x] <= a[x + 1])return 0;
		else if(a[x - 1] > a[x] && a[x] <= a[x + 1])return x - 1;
		else return x;
	};
	SegmentTree<Info> tr(n);
	for(int i = 1; i <= n; i++){
		// dbg(i, getVal(i));
		tr.update(i, {getVal(i)});
	}
	// dbg("????");
	cout << cnt[tr.info[1].val] << '\n';
	for(int i = 1; i <= q; i++){
		int x, y; cin >> x >> y;
		a[x] = y;
		tr.update(x, {getVal(x)});
		if(x > 1)tr.update(x - 1, {getVal(x - 1)});
		if(x < n)tr.update(x + 1, {getVal(x + 1)});
		cout << cnt[tr.info[1].val] << '\n';
	}
    return ;
}
