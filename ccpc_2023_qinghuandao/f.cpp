...
void solve(){
	...
	dp[0][1]=0;
	dp[1][1]=1;
	// int mi11=0,mi12=0,mi21=0,mi22=0;
	// for(int i=2;i<=n;i++){
	// 	dp[1][i]=min(dp[0][i-1],dp[1][i-1])+1;
	// 	int sum=a[i]+a[i-1];
	// 	dbg(i,sum);
	// 	if(!vis[sum])dp[0][i]=dp[0][i-1];
	// 	if(i>=3){
	// 		int d=abs(a[i]-a[i-2]);
	// 		int ok=0;
	// 		if(d%2==0)ok=1;
	// 		else{
	// 			if(a[i]==1&&!vis[a[i-2]+1]){
	// 				ok=1;
	// 			}else if(a[i-2]==1&&!vis[a[i]+1])ok=1;
	// 		}
	// 		if(ok)dp[0][i]=min({dp[0][i],dp[1][i-1]});
	// 		// dp[0][i]=min(dp[0][i],min(dp[0][i-3],dp[1][i-3])+2);
	// 	}else if(i==2){
