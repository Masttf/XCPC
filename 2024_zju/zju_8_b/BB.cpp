    ...
    while (q--) {
        ...
        if (op == 1) {
            ...
            if (a[x] < a[x + 1]) {
                ...
                ansR.add(1, pos + 1, x, -1);
                
                pos = getFirstPos(x + 2, n, a[x]);
                if(pos==0)ansR.update(1,x+1,0);
                else ansR.update(1, x+1,ansR.getSum(1,pos,pos)+1);

                ansL.update(1, x, ansL.getSum(1,x+1,x+1));

                pos = getFirstPos(x + 2, n, a[x] - 1);
                if (pos == 0) pos = n + 1;
                ansL.add(1, x + 1, pos - 1, 1);
            } else {
                
                ansR.update(1,x+1,ansR.getSum(1,x,x));

                int pos = getLastPos(1, x - 1, a[x + 1] - 1);
                ansR.add(1, pos + 1, x, 1);

                pos = getLastPos(1,x-1,a[x+1]);
                if(pos==0)ansL.update(1,x,0);
                else ansL.update(1,x,ansL.getSum(1,pos,pos)+1);
            ...
            if(a[x]<a[x+1]){
                ...
                if(nex[x]&&st.getMaxn(1, x + 2, nex[x] - 1) < a[x]){
                    if(x+2<=nex[x]-1)
                        ansMid.add(1, x + 2, nex[x] - 1, -1);
                }

                if(last[x+1]&&st.getMaxn(1, last[x + 1] + 1, x-1) < a[x + 1])
                    ansMid.add(1, x, x, 1);
                
                if(nex[x+1]&&st.getMaxn(1, x + 2, nex[x + 1] - 1) < a[x + 1])
                    ansMid.add(1, x+1, x+1, -1);
