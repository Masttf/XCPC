...
void solve(){
	...
	//bulid(bulid,1,n,1);
	auto pf = [&]()->void{
		vector<int>temp2(n+1),temp1(n+1);
		vector<int>stk;
		for(int i=1;i<=n;i++){
	...
	for(int i=1;i<=q;i++){
		...
		if(op==2){
			...
			if(a[x]>a[x+1]){
				...
				r=query4(query4,0,n+1,x+2,n+1,a[x],1);
				int d=0;
			...
			if(a[x]<a[x+1]){
				...
				if(last[x+1]&&last[x+1]==l)updata(updata,1,n,x,x,1,2,1);
				
				if(nt[x+1]!=n+1&&nt[x+1]==r)updata(updata,1,n,x+1,x+1,-1,2,1);
				
			}else{
				int l=query1(query1,0,n+1,0,x-1,a[x],1);
				if(last[x]&&last[x]==l)updata(updata,1,n,x,x,-1,2,1);
				
				int r=query3(query3,0,n+1,x+2,n+1,a[x],1);
				if(nt[x]!=n+1&&nt[x]==r)updata(updata,1,n,x+1,x+1,1,2,1);
