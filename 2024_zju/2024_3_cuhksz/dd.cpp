	...
	auto dfs = [&](auto self,int now,int sum,int last,int stg)->void{
		if(now==n+1){
			...
			for(int i=stg+1;i<=n;i++){
				...
				while(len>=(1<<c)){
					// dbg(j,len,(1<<c));
					// dbg(cost[i],pre[j]-pre[j-(1<<c)]);
					res.push_back({cost[i],pre[j]-pre[j-(1<<c)]});
					len-=(1<<c);
					j-=(1<<c);
					c++;
				}
				if(len){
					//dbg(cost[i],j,len,pre[j]-pre[len]);
					res.push_back({cost[i],pre[j]-pre[j-len]});
				}
			}
			for(auto [ct,w]:res){
				for(int j=m1;j>=ct;j--){
					dp[j]=max(dp[j],dp[j-ct]+w);
				}
			}
