...
void solve(){
    ...
    auto merge = [&](auto self, int l, int r, int x, int y, int f) -> int{
	    ...
	    int rt = ++tot;
	    if(l == r){
	    	if(l < f)ans += 1ll * tr[x].cnt * tr[y].cnt;
	...
	auto split = [&](auto self, int l, int r, int x, int y) -> int{
	   	if(tr[x].cnt - tr[y].cnt == 0)return 0;
	   	if(tr[y].cnt == 0)return x;
	    int mid = (l + r) >> 1;
	    int rt = ++tot;
	    if(rt >= 100 * n){
	    	while(1);
	...
	auto dfs2 = [&](auto self, int now, int father) -> void{
		for(auto v : g[now]){
			...
			int ln = root[now];
			int lv = root[v];
			root[now] = split(split, 1, n, root[now], root[v]);
			root[v] = merge(merge, 1, n, root[now], root[v], a[v]);
			dbg(now, v, ans);
			self(self, v, now);
			root[v] = lv;
			root[now] = ln;
