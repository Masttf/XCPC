...
struct Info{
    ...
    void apply(Tag t){
    	for(int i = 0; i <= 30; i++){
    		sum[i] += cnt0[i] * t.c1[i] + cnt1[i] * t.c0[i];
    	}
    }
};
Info operator+ (const Info &a, const Info &b){
    Info res  = Info();
    for(int i = 0; i <= 30; i++){
		res.cnt0[i] = a.cnt0[i] + b.cnt0[i];
		res.cnt1[i] = a.cnt1[i] + b.cnt1[i];
...
void solve(){
	...
	for(int i = n; i >= 1; i--){
		...
		tr.rangeUpdate(i, n, temp);
		for(auto [r, id] : Q[i]){
			Info x = tr.rangeQuery(i, r);
			for(int j = 0; j <= 30; j++){
				ans[id] += mint(x.sum[j]) * (1 << j);
			}
		}
	}
