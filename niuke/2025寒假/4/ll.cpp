...
struct Info{
    ...
    ll sum = 0;
    void apply(Tag t){
        sum += 1ll * cnt0 * t.c1 + 1ll * cnt1 * t.c0;
    }
};
...
void solve(){
    ...
    vector prea(2, vector<int>(n + 5));
    vector preb(2, vector<int>(n + 5));
    vector<int> res(n + 5);
    auto run = [&](int x) -> void{
        for(int i = 1; i <= n; i++){
            prea[0][i] = prea[1][i] = 0;
        ...
        for(int i = 1; i <= q; i++){
            auto [l, r] = Q[i];
            int num = res[l] - res[r + 1];
            num -= (prea[0][l] - prea[0][r + 1]) * (preb[1][r + 1]);
            num -= (prea[1][l] - prea[1][r + 1]) * (preb[0][r + 1]);
            ans[i] += mint(num) * (1 << x);
        }
    };
    for(int i = 0; i <= 30; i++){
