...
struct SegmentTree{
	...
	void rangeUpdate(int l, int r, int x, int y, int p){
		if(x <= l && r <= y){
			if(info[p].sum == info[p].len)return ;
			if(l == r){
				info[p].sum = sqrt(info[p].sum);
				return ;
			}
		}
		int mid = (l + r) >> 1;
		if(x <= mid)rangeUpdate(l, mid, x, y, p << 1);
		if(y > mid)rangeUpdate(mid + 1, r, x, y, p << 1 | 1);
		up(p);
		return ;
	}
	void rangeUpdate(int l, int r){
		return rangeUpdate(1, n, l, r, 1);
	}
};
struct Info{
	int sum = 0;
