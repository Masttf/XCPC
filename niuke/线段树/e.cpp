...
struct Info{
	...
	double cosSum = 0;
    void apply(Tag t){
		double csum =cosSum, ssum = sinSum;
	    sinSum = cos(t.add) * ssum + sin(t.add) * csum;
	    cosSum = cos(t.add) * csum - sin(t.add) * ssum;
    }
};
Info operator+ (const Info &a, const Info &b){
    Info res  = Info();
    res.sinSum = a.sinSum + b.sinSum;
    res.cosSum = a.cosSum + b.cosSum;
    return res;
}
void solve(){
	int n; cin >> n;
	...
	for(int i = 1; i <= q; i++){
		...
		if(ty == 1){
			...
			int l, r; cin >> l >> r;
			cout << fixed << setprecision(1) << tr.rangeQuery(l, r).sinSum << '\n';
		}
	}
    return ;
}
