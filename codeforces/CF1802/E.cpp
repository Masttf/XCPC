	...
	for(int i = 1; i <= n; i++){
		...
		for(auto x : a[now]){
			if(x == 1)mx = sz;
				...
				mx = max(mx, tr.rangeQuery(last, x - 1).mx + sz);
			}
			last = x;
			sz--;
		}
		int res = tr.rangeQuery(a[now].back(), a[now].back()).mx;
		res = max(mx, res);
		tr.update(a[now].back(),{res});
	}
	cout << tr.info[1].mx << '\n';
    return ;
}
...
/*
任意顺序 求最长上升子序列
board1 如果都只有一个元素，直接从小到大排序
按 mx 升序排， 
dpij 表示前i个 最大值是j的最长上升子序列
*/