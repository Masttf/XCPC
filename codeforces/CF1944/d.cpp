...
void solve(){
	...
	auto get = [&](int t,int l,int r)->pair<int,int>{
		if(t==1){
			int d1=(f[r]+(MOD_1-f[l-1])*p[r-l+1]%MOD_1)%MOD_1;
			int d2=(f2[r]+(MOD_2-f2[l-1])*p2[r-l+1]%MOD_2)%MOD_2;
			return {d1,d2};
		}else{
			int d1=(f3[l]+(MOD_1-f3[r+1])*p[r-l+1]%MOD_1)%MOD_1;
			int d2=(f4[l]+(MOD_2-f4[r+1])*p2[r-l+1]%MOD_2)%MOD_2;
			return {d1,d2};
		}
	};
