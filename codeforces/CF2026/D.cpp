...
void solve(){
	...
	for(int i = 1; i <= q; i++){
		int l, r; cin >> l >> r;
		int rl = get(l);
		int num = (n + n - rl + 1) * rl / 2;
		Q[rl + 1].push_back({rl + l - num, i, 1});
		if(r != n * (n + 1) / 2){
			int rr = get(r + 1);
			num = (n + n - rr + 1) * rr / 2;
			Q[rr + 1].push_back({rr + r - num + 1, i, -1});
		}
	}
	...
	int sum = 0;
	for(int i = n; i >= 1; i--){
		tr.rangeUpdate(i, n, {a[i]});
		for(auto [y, id, f] : Q[i]){
			ans[id] += f * (tr.rangeQuery(y, n).sum + sum);
		}
		sum += tr.rangeQuery(i, n).sum;
	}
	for(int i = 1; i <= q; i++){
		cout << ans[i] << '\n';
	}
