...
void solve(){
    ...
    auto split = [&](auto self, int l, int r, int x, int &u, int &v) -> void{
        if(x == 0)return ;
        if(u == useless)return ;
        if(l == r){
    		v = u;
        ...
        tr[v].lson = 1;
        tr[v].rson = 1;
        if(!tr[u].lson)tr[u].lson = newNode(l, mid, u);
        if(!tr[u].rson)tr[u].rson = newNode(mid + 1, r, u);
 		...
 		if(x < val){
            ...
            tr[u].lson = useless;
            father[tr[v].lson] = v;
            self(self, mid + 1, r, x - val, tr[u].rson, tr[v].rson);
            father[tr[v].rson] = v;
 		}
    ...
    for(int i = 1; i <= n; i++){
        ...
        if(ty == 1){
            ...
            split(split, L, R, x, root[rt], root[++id]);
            who[root[id]] = id;
        }else if(ty == 2){
