...
struct Tag{
	int ad, as;
	int had, has;
	Tag() : ad(0), as(-Max), had(-Max), has(-Max){}
	Tag(int ad_, int as_, int had_, int has_) : ad(ad_), as(as_), had(had_), has(has_){}
    void apply(Tag t){
        int had_ = max({-Max, ad + t.had, had});
        int ad_ = max(-Max, ad + t.ad);
        int as_ = max({-Max, as + t.ad, t.as});
        int has_ = max({as + t.had, has, t.has, -Max});
        had = had_;
        ad = ad_;
        as = as_;
        has = has_;
...
void solve(){
	...
	while(q--){
		...
		if(t == 'Q'){
			...
			tr.rangeUpdate(l, r, {val, -Max, val, -Max});
		}else{
			int l, r, val; cin >> l >> r >> val;
			tr.rangeUpdate(l, r, {-Max, val, -Max, val});
		}
	}
    return ;
}
