...
void solve(){
	...
	auto query = [&](auto self,vector<int>&u,vector<int>&v,int l,int r,int p)->int{
		if(l==r){
			int res=0;
			for(auto x:u)res+=tr[x].sum;
			for(auto x:v)res-=tr[x].sum;
			return res;
		}
		int mid=(l+r)>>1;
		if(p<=mid){
			...
			return self(self,u,v,l,mid,p);
		}else{
			for(auto &x:u)x=tr[x].r;
			for(auto &x:v)x=tr[x].r;
			return self(self,u,v,mid+1,r,p);
		}
	};
