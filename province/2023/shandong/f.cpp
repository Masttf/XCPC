...
void solve(){
    ...
    LazySegmentTree<Info, Tag> tr0(n);
    LazySegmentTree<Info, Tag> tr1(n);
    tr0.update(0, {1});
    tr1.update(0, {1});
    vector<mint> dp(n + 1);
    for(int i = 1; i <= n; i++){
    	auto [l, r, c] = a[i];
    	int v = lower_bound(b.begin() + 1, b.end(), l) - b.begin();
    	// dbg(v);
    	if(c == 1){
    		dp[i] = tr0.rangeQuery(0, v - 1).val;
    		tr0.rangeUpdate(0, v - 1, {2});
    		tr1.update(i, {dp[i]});
    	}else{
    		dp[i] = tr1.rangeQuery(0, v - 1).val;
    		tr1.rangeUpdate(0, v - 1, {2});
    		tr0.update(i, {dp[i]});
    	}
    	// dbg(dp[i]);
    }
